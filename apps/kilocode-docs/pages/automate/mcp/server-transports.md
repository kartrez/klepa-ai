...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Klepa AI and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Klepa AI) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Klepa AI
...
- Single-client scenarios (one Klepa AI instance per server)
...
1. The client (Klepa AI) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Klepa AI
...
STDIO servers run locally on the same machine as Klepa AI, which has several important implications:
...
- **Execution**: Starts and stops with Klepa AI (child process lifecycle)
...
- Start when needed by Klepa AI
- Not require network configuration
- Need to be installed alongside Klepa AI or via a package manager
...
## Configuring Transports in Klepa AI

For detailed information on configuring STDIO and SSE transports in Klepa AI, including example configurations, see the [Understanding Transport Types](using-in-kilo-code#understanding-transport-types) section in the Using MCP in Klepa AI guide.
