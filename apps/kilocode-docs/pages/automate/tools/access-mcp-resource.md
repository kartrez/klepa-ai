...
The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows Klepa AI to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When Klepa AI needs additional context from external systems
- When Klepa AI needs to access domain-specific data from specialized MCP servers
- When Klepa AI needs to retrieve reference documentation hosted by MCP servers
- When Klepa AI needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
    ...
    - Returns the processed resource data to Klepa AI for use in the current task
...
- When helping with API development, Klepa AI retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, Klepa AI accesses current data samples from connected MCP servers.
- When working in specialized domains, Klepa AI retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, Klepa AI references compliance requirements from documentation resources.
