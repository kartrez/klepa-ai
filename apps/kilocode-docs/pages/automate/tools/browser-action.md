...
The `browser_action` tool enables web automation and interaction via a Puppeteer-controlled browser. It allows Klepa AI to launch browsers, navigate to websites, click elements, type text, and scroll pages with visual feedback through screenshots.
...
This tool creates an automated browser session that Klepa AI can control to navigate websites, interact with elements, and perform tasks that require browser automation. Each action provides a screenshot of the current state, enabling visual verification of the process.

## When is it used?

- When Klepa AI needs to interact with web applications or websites
...
- Browser sessions are temporary and not persistent across Klepa AI restarts
...
- When creating a web form submission process, Klepa AI launches a browser, navigates to the form, fills out fields with the `type` action, and clicks submit.
- When testing a responsive website, Klepa AI navigates to the site and uses scroll actions to examine different sections.
- When capturing screenshots of a web application, Klepa AI navigates through different pages and takes screenshots at each step.
- When demonstrating an e-commerce checkout flow, Klepa AI simulates the entire process from product selection to payment confirmation.
