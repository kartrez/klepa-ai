---
title: "Skills"
description: "Extend Klepa AI capabilities with skills"
---

# Skills

Klepa AI implements [Agent Skills](https://agentskills.io/), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
1. **Discovery**: Skills are scanned from designated directories when Klepa AI initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
When multiple skills share the same name, Klepa AI uses these priority rules:
...
Skills are discovered when Klepa AI initializes:

- When VSCode starts
- When you reload the VSCode window (`Cmd+Shift+P` → "Developer: Reload Window")

Skills directories are monitored for changes to `SKILL.md` files. However, the most reliable way to pick up new skills is to reload VS or the Klepa AI extension.
...
In Klepa AI, the `name` field **must match** the parent directory name:
...
- **Kilo Marketplace** - Browse skills directly in the Klepa AI extension via the Marketplace tab, or explore the [Kilo Marketplace repository](https://github.com/Kilo-Org/kilo-marketplace) on GitHub
...
1. **Check the Output panel**: Open `View` → `Output` → Select "Klepa AI" from dropdown. Look for skill-related errors.
...
For more details on contributing to Klepa AI, see the [Contributing Guide](/docs/contributing).
