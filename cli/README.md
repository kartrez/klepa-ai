...
Terminal User Interface for Klepa AI
...
Then, make sure you place your Klepa AI API token in the CLI config:

```bash
kilocode config # this opens up your editor
```

You can find your Klepa AI API token on your profile page at [app.gpt-chat.by](https://app.gpt-chat.by), and place it in the `kilocodeToken` field in the CLI config.
...
When installing Klepa AI CLI you'll be greeted by some scary looking dependency deprecation warnings. We're aware of the issue and will resolve it shortly.
...
Parallel mode allows multiple Klepa AI instances to work in parallel on the same directory, without conflicts. You can spawn as many Klepa AI instances as you need! Once finished, changes will be available on a separate git branch.
