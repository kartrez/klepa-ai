...
export class CLI {
	...
	async initialize(): Promise<void> {
		...
		try {
			logs.info("Initializing Klepa AI CLI...", "CLI")
			...
			if (supportsTitleSetting()) {
				process.stdout.write(`\x1b]0;Klepa AI - ${folderName}\x07`)
			}
			...
			logs.info("Klepa AI CLI initialized successfully", "CLI")
		} catch (error) {
	...
	async dispose(signal?: string): Promise<void> {
		...
		try {
			logs.info("Disposing Klepa AI CLI...", "CLI")
			...
			logs.info("Klepa AI CLI disposed", "CLI")
		} catch (error) {
