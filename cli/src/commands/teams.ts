...
async function listTeams(context: CommandContext): Promise<void> {
	...
	if (organizations.length < 1) {
		addMessage({
			id: Date.now().toString(),
			type: "system",
			content: `You're currently not a part of any Klepa AI teams. Go to https://gpt-chat.by/get-started/teams to get started with Klepa AI for Teams!`,
			ts: Date.now(),
		})
		return
	}
...
export const teamsCommand: Command = {
	...
	arguments: [
		...
		},
		{
			name: "team-name",
			description:
				"Team name in lowercase with dashes (e.g., 'kilo-code' for 'Klepa AI') or 'personal' (for select subcommand)",
			required: false,
