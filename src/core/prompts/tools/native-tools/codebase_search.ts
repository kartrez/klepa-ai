...
const QUERY_PARAMETER_DESCRIPTION = `The search query. Reuse the user's exact wording/question format unless there's a clear reason not to.`

const PATH_PARAMETER_DESCRIPTION = `Optional subdirectory (relative to the current workspace directory) to limit the search scope. Leave null for entire workspace.`
