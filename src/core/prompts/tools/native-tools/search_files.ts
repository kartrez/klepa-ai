...
const PATH_PARAMETER_DESCRIPTION = `The path of the directory to search in (relative to the current workspace directory). This directory will be recursively searched.`

const REGEX_PARAMETER_DESCRIPTION = `The regular expression pattern to search for. Uses Rust regex syntax.`

const FILE_PATTERN_PARAMETER_DESCRIPTION = `Optional glob pattern to filter files (e.g., '*.ts' for TypeScript files). If not provided, it will search all files (*).`
