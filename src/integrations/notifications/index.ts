...
async function showMacOSNotification(options: NotificationOptions): Promise<void> {
	...
	try {
		...
		const iconPath = vscode.Uri.joinPath(extensionUri, "assets", "icons", "icon-dark.png").fsPath
...
"@
    ...
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Klepa AI").Show($toast)
    `
...
export async function showSystemNotification(options: NotificationOptions): Promise<void> {
	try {
		const { title = "Klepa AI", message } = options
